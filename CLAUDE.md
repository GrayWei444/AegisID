...
### 2.6b 唯一 ID 優化計畫（v15 — 待實作） ⚠️

**問題：目前 face_hash 不穩定 — 同一人每次掃描產生不同 hash → VPS 帳號恢復失敗**

已知原因：
- 3D bone ratio bins 在不同光線/角度下量化結果不一致
- BIN_WIDTH=0.25 + floor-biased rounding 容易跨 bin 邊界

**v15 優化方向：2D 特徵 + 3D 特徵（重新篩選）+ PIN 聯合判定**

```
目前 v14:
  face_hash = SHA-256(25 個 3D bone ratio bins)
  account_key = SHA-256(face_hash + PIN)

v15 目標:
  2d_hash = SHA-256(2D 正面特徵 bins)        ← 新增
  3d_hash = SHA-256(重新篩選的 3D 特徵 bins)  ← 優化
  combined_hash = SHA-256(2d_hash + 3d_hash)  ← 聯合
  account_key = SHA-256(combined_hash + PIN)  ← 最終唯一 ID
```

**具體改動：**

1. **2D 正面特徵 hash 化**
   - 從正面幀提取穩定的 2D 比率（眼距/鼻寬/臉寬等）
   - 不依賴 3D 重建，直接從 landmark 計算
   - 量化方式可能需要調整（更大的 BIN_WIDTH 增加穩定性）

2. **3D 特徵重新篩選**
   - 分析 67 個比率的跨次穩定性，篩掉不穩定的
   - 可能從 25 個減少到 15-20 個最穩定的
   - 考慮改用 round() 而非 floor-biased（已是 round 但需驗證）

3. **聯合判定**
   - 2D hash + 3D hash 聯合 → 提高唯一性
   - 單獨 2D 或 3D 不夠唯一，聯合後更強
   - 容錯：2D 和 3D 各自允許少量 bin 不同（fuzzy match）

4. **VPS 查找改為 fuzzy match**
   - 目前：exact match（account_key 完全一致）
   - 可能改為：2D hash 前 N bits match + 3D hash 前 M bits match
   - 或保持 exact match 但提高 hash 穩定性

**驗證方式：**
- 同一人 10 次掃描 → 10 次 hash 必須完全一致
- 不同人 → hash 必須不同
- 雙胞胎 + 不同 PIN → account_key 不同

**相關檔案：**
- `sdk/src/face/structuralId.ts` — computeStructuralId, bone ratio 計算
