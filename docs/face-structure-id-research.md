...
| Laplacian 曲率場而非絕對深度 | 對距離和全局偏移天然不變（但深度路線最終仍失敗）|

---

## 12. 2026-03-20 更新：研究進展

### 12.1 影像正規化管線（已驗證 ✅）

最終採用的管線（與早期研究文件不同，棄用了 CLAHE 和 Gaussian blur）：

```
ArcFace 5pt align → Grayscale → 全局直方圖均衡化 → 橢圓臉部遮罩 → 112×112
```

| 早期方案 | 最終方案 | 原因 |
|---------|---------|------|
| CLAHE | 全局 histEq | CLAHE 分區處理扭曲五官邊界 |
| Gaussian blur σ=1 | 不模糊 | SSIM 0.993 無需模糊，模糊反而丟資訊 |
| Min-max normalize | 不需要 | histEq 本身已映射到 0-255 |
| 無遮罩 | 橢圓遮罩 | 背景是最大干擾源 |

**關鍵發現**: 背景（天花板/牆壁）是圖像不穩定的最大原因，不是光線。
橢圓遮罩消除背景後，全局 histEq 就足以達到 SSIM 0.993。

### 12.2 深度路線（已放棄 ❌）

**測試數據**: Depth Anything V2 + Laplacian 曲率場
- 穩定性: 12.3/48 (26%) — 等於隨機
- 原因: 深度模型推論不確定性 → 二階微分放大誤差
- bin width 0.002 太細，但根本問題是底層模型不穩定
- 手機端: ~12,600ms WASM + OOM 風險

**結論**: 單目深度估測在手機端不可行，放棄此路線。

### 12.3 骨骼比率（重新評估，大量擴充中 🔄）

早期結論是 landmark 比率不穩定。但重新分析後發現：

1. 12 個比率中有 10 個是穩定的 (83%)
2. 不穩定的 2 個 (r03, r11) 可以識別並淘汰
3. MediaPipe 有 468 個 landmarks，組合空間巨大
4. ArcFace 對齊後的座標比原始座標穩定得多

**新策略**: 大量擴充 → 嚴格篩選

擴充歷程：
- 初始: 12 個比率
- 第一次擴充: 30 個（加入鼻部、眼睛幾何、眉毛、嘴巴、下顎顴骨）
- 第二次擴充: 67 個（系統性覆蓋所有面部區域 + 交叉特徵）
- 計劃: 繼續擴充到 100+，然後通過多條件測試篩選
...
| 大小 | 穩定性 | 區分力 | 用途 |
|------|--------|--------|------|
| 4×4 (15 bit) | 100% ✅ | 32K 組合（不足）| 快速預篩 |
| 8×8 (63 bit) | 2-4 bit 漂移 | 2^63（足夠）| 暫不使用 |

**觀察**: #2,#3 的 pHash 是 8666，#4,#5 是 8276 — 中間有一次微動導致變化。
但 4×4 level 完全一致，說明微動只影響高頻（8×8 的額外 bit）。

### 12.5 組合策略演進

```
早期想法（已修正）:
  Gabor filter + LBP + 深度曲率 → hash
  問題: Gabor/LBP 未測試，深度已失敗

當前方向:
  pHash 4×4 (預篩) + 穩定骨骼比率 (唯一 hash) + CNN (輔助)
  依據: pHash 和骨骼比率已有實測數據支持

待測試:
  Gabor filter bank — 仍然是有潛力的確定性特徵，但優先級低於骨骼比率
  LBP — 對雜訊敏感，且需要的資訊量可能骨骼比率已覆蓋
```

### 12.6 下一步測試計劃

1. **67 比率 L1 穩定性** — 頭不動連拍，找出穩定子集
2. **轉頭測試** — ±10°, ±20°, 看哪些比率對角度敏感
3. **不同距離** — 30cm vs 50cm vs 80cm
4. **不同裝置** — Samsung Z Fold vs iPhone vs 其他 Android
5. **最終篩選** — 保留所有條件下都穩定的 20-30 個比率
